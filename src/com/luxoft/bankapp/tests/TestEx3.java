...
public class TestEx3 {
    ...
    public void testBankReportStreams() throws ClientExistsException {
        ...
        BankReportStreams reportStreams1 = new BankReportStreams();

        //Testing

        //Get number of clients test
        assertEquals(3,reportStreams1.getNumberOfClients(bank));

        //Get number of accounts test
        assertEquals(6, reportStreams1.getNumberOfAccounts(bank));

        //Display clients sorted by name alphabetically
        reportStreams1.getClientsSorted(bank);

        //Get total sum in accounts in the bank
        //ToDo: trebuia si overdraftul?? in suma
        //delta = precizia
        assertEquals(14500,reportStreams1.getTotalSumInAccounts(bank),0.01);
        ...
        assertEquals(expectedAccountsSorted,reportStreams1.getAccountSortedBySum(bank));

        //Get the sum of all bank credits provided by the bank
        assertEquals(600,reportStreams1.getBankCreditSum(bank),0.01);
        ...
        assertEquals(accMap,reportStreams1.getCustomerAccounts(bank));

        //Display clients for each city(cities in alphabetical order)
        reportStreams1.getClientsByCity(bank);
    }
